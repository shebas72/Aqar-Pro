...
class _DealDetailPageState extends State<DealDetailPage> {
  ...
  loadData() {
    ...
    if(!nextActionOptionsList.contains(_nextActionValue)) {
      _nextActionValue = "";
    }

    if(!statusOptionsList.contains(_statusValue)) {
      _statusValue = "";
    }
  ...
  fetchNonce() async {
    ApiResponse response = await _apiManagerCRM.fetchDealDeleteNonceResponse();
    if (response.success) {
  ...
  Future<void> _selectDateActionDue(BuildContext context, String key) async {
    ...
    if (picked != null && picked != actionDueSelectedDate) {
        ...
        await _apiManagerCRM.updateDealDetails(updateData);
              ...
              actions: <Widget>[
                ...
                TextButtonWidget(
                  onPressed: () async {
                    ...
                    final response = await _apiManagerCRM.deleteDeal(dealID, nonce);
                    if (response.success) {
                      Navigator.pop(context);
                      Navigator.pop(context);
