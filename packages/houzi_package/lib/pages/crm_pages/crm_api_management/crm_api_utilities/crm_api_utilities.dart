...
class CRMApiUtilities {
  ...
  Dio getDio(bool forceExcludeCache) {
  ...
  refreshToken() async {
    ApiMangerCRM _apiManagerCRM = ApiMangerCRM();
    ...
    if (_userInfo.isNotEmpty) {
      ...
      String nonce = "";
      ApiResponse<String> nonceResponse = await _apiManagerCRM.fetchSignInNonceResponse();
      ...
      if (nonceResponse.success) {
        ...
        if (_userInfo.containsKey(USER_SOCIAL_PLATFORM)) {
          response = await _apiManagerCRM.socialSignOn(_userInfo, nonce);
        } else {
          response = await _apiManagerCRM.login(_userInfo, nonce);
        }

        if (response.success && response.internet && response.result != null) {
          UserLoginInfo info = response.result!;
          Map<String, dynamic> userLoginData = _apiManagerCRM.convertUserLoginInfoToJson(info);
      ...
      if(dioError.response != null){
        ...
        if(handle403){
          if(dioError.response!.statusCode! == 403 || dioError.response!.statusCode! == 401){
            if (tries <= MAX_TRIES) {
              refreshToken();
              return doRequestOnRoute(uri, type:type, formParams: formParams, tag: tag, tries: ++tries);
            }
          }
        }
