...
class CRMHouzezParser implements CRMApiParser {
  ...
  @override
  ApiResponse<String> parseNonceResponse(Response<dynamic> response) {
    return CRMHouzezParser.parseNonceResponseFunc(response);
  }

  @override
  parseApi(ApiRequest<dynamic> request, Response<dynamic> response) {
    return CRMHouzezParser.parseApiFunc(request, response);
  }
    ...
    if (response.statusCode == HttpStatus.ok) {
      if (response.data is Map) {
        ...
        if (map != null && map.containsKey("success") && map["success"]) {
          if (map.containsKey("nonce")) {
            return ApiResponse<String>(internet: true, success: true, result: map["nonce"], message: "success");
          }
        }
      }
    } else {
      ...
      if (response.statusCode == 403 && response.data is Map) {
        ...
        if (map != null && map.containsKey("reason") && map["reason"] != null) {
          ...
          return ApiResponse<String>(internet: true, success: false, result: "", message: map["reason"]);
        }
      }
    }
    return ApiResponse<String>(internet: true, success: false, result: "", message: response.statusMessage!);
  }
    ...
    if (map != null) {
      if (response.statusCode == HttpStatus.ok || response.statusCode == HttpStatus.internalServerError) {
        if (map.containsKey("success") && map["success"]) {
          ...
          return ApiResponse<String>(success: true, internet: true, result: result, message: message);
        } else if (map.containsKey("reason") && map["reason"] != null) {
          UtilityMethods.printAttentionMessage(map["reason"]);
          return ApiResponse<String>(success: false, internet: true, result: map["reason"] as String, message: map["reason"] as String);
        } else if (map.containsKey("remove_attachment")) {
          String _message = "";
          if (map["remove_attachment"] != null && map["remove_attachment"] is Map) {
            _message = "Image removed successfully!";
          } else {
            _message = "No Image removed!";
          }
          return ApiResponse<String>(success: true, internet: true, result: "", message: _message);
        } else if (map.containsKey("msg") && map["msg"] != null) {
          String message = map["msg"] as String;
          return ApiResponse<String>(success: false, internet: true, result: message, message: message);
        } else if (map.containsKey("message") && map["message"] != null) {
        ...
        if (map.containsKey("reason") && map["reason"] != null) {
          UtilityMethods.printAttentionMessage(map["reason"]);
          return ApiResponse<String>(success: false, internet: true, result: "", message: map["reason"] as String);
        }
      }
    }

    return ApiResponse<String>(success: false, internet: true, result: "", message: response.statusMessage ?? "Unknown error");
  }
  ...
  static dynamic parseApiFunc(ApiRequest request, Response response) {
    ...
    switch(tag) {
      case CRMActivitiesTag: {
        rawData = response.data["results"];
        List data = [];
        data.addAll(rawData.map((m) => parseActivitiesMap(m)).toList());
        return data;
      }
      ...
      case CRMLeadsTag: {
        ...
        List data = [];
        data.add(parseLeadsMap(rawData));
        return data;
      }

      case CRMActivityDealsTag: {
        rawData = response.data;
        List data = [];
        data.add(parseDealsFromActivityMap(rawData));
        return data;
      }

      case CRMBoardLeadsTag: {
        rawData = response.data;
        List data = [];
        data.add(parseDealsAndLeadsFromActivityMap(rawData));
        return data;
      }

      case CRMInquiriesTag: {
        rawData = response.data["results"];
        List data = [];
        data.addAll(rawData.map((m) => parseInquiriesMap(m)).toList());
        return data;
      }

      case CRMInquiryNotesTag: {
        rawData = response.data;
        List data = [];
        data.addAll(rawData.map((m) => parseInquiryNotesMap(m)).toList());
        return data;
      }

      case CRMLeadInquiriesTag: {
        rawData = response.data["enquiries"];
        List data = [];
        data.addAll(rawData.map((m) => parseInquiriesMap(m)).toList());
        return data;
      }

      case CRMLeadDetailsTag: {
        rawData = response.data;
        List data = [];
        if (rawData != null && rawData is Map) {
          data.add(parseDealsFromBoardMap(response.data));
        }
        return data;
      }

      case CRMLeadViewedTag: {
      ...
      case CRMGetLeadsTag: {
        rawData = response.data["results"];
        List data = [];
        data.addAll(rawData.map((m) => parseDealsFromBoardMap(m)).toList());
        return data;
      }
      ...
      case CRMGetDealsTag: {
        ...
        List list = [];
        list.addAll(rawData.map((m) => parseDealsFromBoardMap(m)).toList());
        data["list"] = list;
        data["status"] = response.data["status"];
        data["actions"] = response.data["actions"];
        return data;
      }
