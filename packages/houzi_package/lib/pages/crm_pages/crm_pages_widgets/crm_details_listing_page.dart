...
class _CRMDetailsListingState extends State<CRMDetailsListing> {
  ...
  Future<List<dynamic>> fetchInquiryDetailMatchedFromBoard() async {
    ...
    if (widget.fetch == FETCH_INQUIRY_MATCHING) {
      response = await _apiManagerCRM.fetchInquiryDetailMatchedFromBoard(widget.id, page);
    } else if (widget.fetch == FETCH_LEAD_INQUIRY) {
      response = await _apiManagerCRM.fetchLeadsInquiriesFromBoard(widget.id, page);
    } else if (widget.fetch == FETCH_LEAD_VIEWED) {
      response = await _apiManagerCRM.fetchLeadsViewedFromBoard(widget.id, page);
    } else if (widget.fetch == FETCH_LEAD_DETAIL) {
      response = await _apiManagerCRM.fetchLeadsInquiriesFromBoard(widget.id, page, fetchLeadDetail: true);
    }
  ...
  onTapEmail() async {
    ...
    ApiResponse<String> response = await _apiManagerCRM.sendCRMEmail(params);
