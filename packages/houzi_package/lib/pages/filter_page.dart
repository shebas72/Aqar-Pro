...
class _FilterPageState extends State<FilterPage>{
  ...
  getFilterPageConfigFile() {
    ...
    String configString = HiveStorageManager.readFilterConfigListData() ?? "";

    if (configString.isNotEmpty) {
      configList = jsonDecode(configString);
    }
    ...
    if (configList.isNotEmpty) {
      ...
      if (mounted) {
        setState(() {
          filterPageConfigElementsList = configList.map<FilterPageElement>((item) =>
              _apiManager.parseFilterPageElementJson(item)).toList();
        });
      }
    }
  }
