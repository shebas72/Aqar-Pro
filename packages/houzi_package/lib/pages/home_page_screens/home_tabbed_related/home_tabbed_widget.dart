...
class _HomeTabbedState extends ParentSliverHomeState<HomeTabbed> with TickerProviderStateMixin {
  ...
  @override
  getHomeConfigFile() {
    List tempList = UtilityMethods.readHomeConfigFile();

    if (tempList.isNotEmpty && mounted) {
      setState(() {
        homeConfigList = tempList.map((item) => _apiManager.convertHomeLayoutToJson(item)).toList();
