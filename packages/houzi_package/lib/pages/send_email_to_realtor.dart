...
class SendEmailToRealtorState extends State<SendEmailToRealtor>
  ...
  loadData() {
    ...
    _isFromAgentProfile = listingId == null;
  }

  fetchNonce() async {
    ApiResponse response = _isFromAgentProfile ?  await _apiManager.fetchContactRealtorNonceResponse() : await _apiManager.fetchContactPropertyRealtorNonceResponse();
    if (response.success) {
  ...
  onSubmitPressed(){
    if (formKey.currentState!.validate()) {
      ...
      String key = widget.informationMap[SEND_EMAIL_REALTOR_EMAIL].toString();
      if (!_isFromAgentProfile) {
        String listingId = widget.informationMap[SEND_EMAIL_LISTING_ID]
            .toString();
        key = listingId;
      }
      key = key + email;
  ...
  void sendEmail() async {
    Map<String, dynamic> _formParams = {
      AgentIdKey : widget.informationMap[SEND_EMAIL_REALTOR_ID].toString(),
      TargetEmailKey: widget.informationMap[SEND_EMAIL_REALTOR_EMAIL],
      NameKey: name,
      EmailKey: email,
      MobileKey: phone,
      MessageKey: message,
      UserTypeKey: _selectedCustomerType!,
      AgentTypeKey: widget.informationMap[SEND_EMAIL_REALTOR_TYPE],
      PropertyIdKey: widget.informationMap[SEND_EMAIL_UNIQUE_ID].toString(),
      ListingIdKey: widget.informationMap[SEND_EMAIL_LISTING_ID].toString(),
      PropertyTitleKey: widget.informationMap[SEND_EMAIL_LISTING_NAME] ?? "",
      PermaLinkKey: widget.informationMap[SEND_EMAIL_LISTING_LINK],
      SourceLinkKey: widget.informationMap[SEND_EMAIL_LISTING_LINK],
      SourceKey: widget.informationMap[SEND_EMAIL_SOURCE],
    };

    ApiResponse<String> response = _isFromAgentProfile ?  await _apiManager.contactRealtor(_formParams, nonce) : await _apiManager.contactPropertyRealtor(_formParams, nonce);
    ...
    if (response.success) {
      ...
      key = key + email;
      HiveStorageManager.storePropertyEmailContactInfo(infoMap, key);
