...
class _AddNewAgentPageState extends State<AddNewAgentPage> with ValidationMixin {
  ...
  fetchNonce() async {
    ...
    if (widget.agentDataForEdit != null) {
      response = await _apiManager.fetchEditAgentNonceResponse();
    } else {
      response = await _apiManager.addAgentNonce();
    }
    if (response.success) {
  ...
  void onSavedButtonPressed() async {
    if (formKey.currentState!.validate()) {
      ...
      if (widget.agentDataForEdit == null) {
        ...
        response = await _apiManager.addAgent(addAgentInfoMap, nonce);
      } else {
        addAgentInfoMap.removeWhere((String key, dynamic value) =>
            ((value == null) || (value.isEmpty)));

        response = await _apiManager.editAgent(addAgentInfoMap, nonce);
      }
