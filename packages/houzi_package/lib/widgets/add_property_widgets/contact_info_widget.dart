...
class _ContactInformationBodyWidgetState extends State<ContactInformationBodyWidget> {
  ...
  void initState() {
    ...
    if (_userRole == USER_ROLE_AGENCY_VALUE) {
      ...
      return Container(
        ...
        child: ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            if (_userRole == USER_ROLE_AGENCY_VALUE && index == 0) {
              return Container();
  ...
  void loadRemainingData() {
    fetchAllAgents(1, 16, true).then((value) {
      ...
      if(_userRole != USER_ROLE_AGENCY_VALUE) {
        fetchAllAgenciesInfo(1, 16,true).then((value) {
          if (mounted && value.isNotEmpty) {
  ...
  assignValues() {
    ...
    } else if (_userRole == USER_ROLE_AGENCY_VALUE) {
      selectedValue = realtorsList[2];
  ...
  Future<List<dynamic>> fetchAllAgents(int page, int perPage, bool? visibilityOnly) async {
    List tempList = [];
    ...
    try {
      do {
        if (_userRole == USER_ROLE_AGENCY_VALUE) {
          if (useAuthorInfo) {
            response = await _apiManager.allAgentOfAnAgency(realtorId!);
          } else {
            response = await _apiManager.fetchAgentOfAnAgency(realtorId!);
          }
        } else {
          response = await _apiManager.fetchAllAgents(page, perPage,visibilityOnly);
        }
  ...
  Future<List<dynamic>> fetchAllAgenciesInfo(int page, int perPage, bool? visibilityOnly) async {
    try {
      if (mounted) {
        List tempList = [];
        do{
          if (_userRole == USER_ROLE_AGENCY_VALUE) {
            ...
            if (agencyUserId != null) {
              ApiResponse<List> response = await _apiManager.singleAgency(agencyUserId);
              if (response.success && response.internet) {
                tempList = response.result;
              }
            }
          } else {
            ApiResponse<List> response = await _apiManager.fetchAllAgencies(page, perPage, visibilityOnly);
            if (response.success && response.internet) {
              tempList = response.result;
            }
          }
